...
#     # test_df[col] = le.transform(test_df[col])  # Use same encoding
#     # Map using existing encoder, fallback to -1 for unknown labels
...
# mse_auto = np.mean(np.power(X_test_scaled - reconstructions, 2), axis=1)
# threshold_auto = np.percentile(mse_auto, 95)
# test_df['is_anomaly_autoencoder'] = (mse_auto > threshold_auto).astype(int)
...
# print("🔍 Applying KNN for anomaly detection...")
# knn = NearestNeighbors(n_neighbors=5)
# knn.fit(X_train_scaled)
# distances, _ = knn.kneighbors(X_test_scaled)

# anomaly_score_knn = distances.max(axis=1)
# threshold_knn = np.percentile(anomaly_score_knn, 95)
# test_df['is_anomaly_knn'] = (anomaly_score_knn > threshold_knn).astype(int)
