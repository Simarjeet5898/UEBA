...
def init_data_exfiltration_table():
    ...
    cursor = conn.cursor()
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS data_exfiltration_config_ueba (
...
def add_data_exfiltration_config(sensitive_files, sensitive_data_types, sensitive_file_extensions, trusted_ip_ranges):
    ...
    cursor.execute("SELECT id FROM data_exfiltration_config_ueba LIMIT 1;")
    row = cursor.fetchone()

    if row:
        config_id = row[0]
        cursor.execute("""
            UPDATE data_exfiltration_config_ueba
        ...
        cursor.execute("""
            INSERT INTO data_exfiltration_config_ueba
...
def get_all_data_exfiltration_configs():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM data_exfiltration_config_ueba ORDER BY id;")
...
def get_data_exfiltration_config(config_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM data_exfiltration_config_ueba WHERE id = %s;", (config_id,))
    row = cursor.fetchone()
    columns = [desc[0] for desc in cursor.description]
...
def delete_data_exfiltration_config(config_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM data_exfiltration_config_ueba WHERE id = %s;", (config_id,))
    conn.commit()
    cursor.close()
    conn.close()
    return True
